...
@interface DetailViewController ()<UIWebViewDelegate,UIActionSheetDelegate,VKSdkDelegate, UINavigationControllerDelegate> {
...
//    height = 0;
    if (IOS_VERSION >= 7.0) {
        ...
        [self.navigationController.navigationBar setBarTintColor:[UIColor colorWithRed:198.0/255.0 green:198.0/255.0 blue:198.0/255.0 alpha:1.0]];
        ...
        [self.navigationController.navigationBar setTintColor:[UIColor colorWithRed:198.0/255.0 green:198.0/255.0 blue:198.0/255.0 alpha:1.0]];
    ...
    self.navigationItem.titleView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"april24logo"]];
...
//    lblName.frame = [lblName.text boundingRectWithSize:CGSizeMake(300, CGFLOAT_MAX) options:(NSStringDrawingUsesLineFragmentOrigin | NSLineBreakByWordWrapping) attributes:@{NSFontAttributeName: lblName.font} context:nil];
//    lblName.frame = CGRectMake(lblName.frame.origin.x, lblName.frame.origin.y , lblName.frame.size.width, lblName.frame.size.height);
  //  NSLog(@"name = %@", [webInfo valueForKey:@"name"]);
...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
    ...
    mainScrollView.contentSize = CGSizeMake(mainScrollView.frame.size.width, webDescriptionView.frame.origin.y + webDescriptionView.frame.size.height-1);
//    if (isCommentAdded) {
